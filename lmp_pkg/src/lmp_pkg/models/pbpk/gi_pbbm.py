...
def create_gi_model(subject, api):
    ...
    if subject.gi:
        ...
        return GIModel(
            ...
            fu_gi=getattr(api, 'fu_gi', 1.0),
            fu_plasma=getattr(api, 'fu_plasma', 0.5),
            bp_ratio=getattr(api, 'blood_plasma_ratio', 0.855)
        )
    else:
...
class GIPBBMParams:
    ...
    effective_permeability_cm_s: float = 1.535e-2
    hepatic_extraction_fraction: float = 0.9
    fu_gi: float = 1.0
    fu_plasma: float = 0.5
