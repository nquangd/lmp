...
class PBBMConfig(BaseModel):
    ...
    model: str = "null"
    epi_layers: List[int] = Field(default_factory=lambda: [2, 2, 1, 1])
...
class RunConfig(BaseModel):
    ...
    stages: List[str] = Field(default_factory=lambda: ["deposition", "pbbm", "pk"])
    seed: int = 123
    threads: int = 1
    ...
    def validate_stages(cls, v: List[str]) -> List[str]:
        valid_stages = {"deposition", "pbbm", "pk", "analysis"}
...
class AppConfig(BaseModel):
    ...
    # Entity references
    subject: EntityRef = Field(default_factory=lambda: EntityRef(ref="adult_70kg"))
    api: EntityRef = Field(default_factory=lambda: EntityRef(ref="salbutamol"))
    product: EntityRef = Field(default_factory=lambda: EntityRef(ref="HFA_MDI_PT210"))
    maneuver: EntityRef = Field(default_factory=lambda: EntityRef(ref="standard_MDI_inhale_hold"))
