...
def ML_CFD_MT_deposition(mmad, gsd, propellant, usp_deposition, pifr, cast, DeviceType):
#def ML_CFD_MT_deposition(self):
    ...
    """
    Calculates Mouth-Throat deposition using a pre-trained Machine Learning model.
    """ 
    ...
    data_path = os.path.join(pkg_dir, '..', 'data', 'cfd', 'DataBase_July_10.csv')
    deposition = pd.read_csv(data_path)
    ...
    # Calculate coarse fraction based on USP
    df = deposition[(deposition['Product'] == propellant) & (deposition['Inlet'] == 'USP') & (deposition['DeviceType'] == DeviceType) ]
    prob = np.array(df['Mouth']) # / (1 - np.array(df['Device']) )
    bins = df['Size'].unique()
    ...
    bins = deposition[(deposition['Product'] == propellant) & (deposition['Inlet'] == cast) &  (deposition['Flowrate'] == 30) &  (deposition['DeviceType'] == DeviceType) ]['Size'].unique()
    
    # Load the ML model
    model_filename = f'Fitted_CFD_{propellant}_{cast}_{DeviceType}.sav'
    model_path = os.path.join(pkg_dir, '..', 'data', 'cfd', model_filename)
    
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"ML model not found at {model_path}")
        
    model = joblib.load(open(model_path, 'rb'))
    prob_cast = model.predict(np.array([bins, [1 / pifr] * len(bins)]).T)
    
    # Re-calculate mf-in with bins for CAST, can either be the same or not the same as bins of USP
    mf_in = Mass_Fraction_From_PSD(psd_in, bins)
    ...
    mmad_cast, gsd_cast = MMADfromCDF(bins, mf_out_cast)
