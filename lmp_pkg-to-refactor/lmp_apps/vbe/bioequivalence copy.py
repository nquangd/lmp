...
from ..efficacy.fev1model import COPD_FEV1

class BioequivalenceAssessor:
    """
    A class to handle bioequivalence (BE) statistical assessments.
    """
    def __init__(self, study, residual_var = 0.013, inner_boot=100, alpha_pk=0.1, alpha_pd=0.1, alpha_metrics=0.05):
        ...
        self.fevmodel = COPD_FEV1(study, residual_var)
        #self.calculate_effects = fevmodel.calculate_effects
    ...
    def systemic_product_metrics(self, df ):
        ...
        for trial in range(self.n_trials):
            sampled_subjects = np.random.choice(systemic['Subject'].unique(), size=self.trial_size, replace=True)
            ...
            for product in products:
                for api in apis:
                    for param in parameters:
                        ...
                        val_mean, mean_low, mean_up = self.bootstrap_ci(values, np.mean, n_bootstrap=self.inner_boot, alpha=self.alpha_metrics)
                        val_geo, geo_low, geo_up = self.bootstrap_ci(values, self.geometric_mean, n_bootstrap=self.inner_boot, alpha=self.alpha_metrics)
                        
                        trials.append({
                            'Trial': trial, 'Product': product, 'API': api,
                            ...
                            'CV_percent': self.cv_percent(values), 'CV_Geo_percent': self.cv_geometric_percent(values)
                        })
        ...
        for (product, api, param), g in trial_df.groupby(['Product', 'API', 'Parameter']):
            ...
            mean, mean_low, mean_up = self.bootstrap_ci_fixed_effects(g['Mean'].values, g['Mean_CI_Lower'].values, g['Mean_CI_Upper'].values, alpha=self.alpha_metrics, logscale=False)
            geo, geo_low, geo_up = self.bootstrap_ci_fixed_effects(g['GeoMean'].values, g['GeoMean_CI_Lower'].values, g['GeoMean_CI_Upper'].values,  alpha=self.alpha_metrics, logscale=False)
            summary.append({
    ...
    def systemic_bioequiv(self, df):
        ...
        for trial in range(self.n_trials):
            ...
            for api, gdf in sys_sample.groupby('API'):
                ...
                auc_m, auc_m_low, auc_m_up = self.ratio_and_ci(test['Systemic_AUC'].values, ref['Systemic_AUC'].values, np.mean, self.inner_boot, alpha= self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                auc_g, auc_g_low, auc_g_up = self.ratio_and_ci(test['Systemic_AUC'].values, ref['Systemic_AUC'].values, self.geometric_mean, self.inner_boot, alpha= self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                cmax_m, cmax_m_low, cmax_m_up = self.ratio_and_ci(test['Systemic_Cmax'].values, ref['Systemic_Cmax'].values, np.mean, self.inner_boot, alpha= self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                cmax_g, cmax_g_low, cmax_g_up = self.ratio_and_ci(test['Systemic_Cmax'].values, ref['Systemic_Cmax'].values, self.geometric_mean, self.inner_boot, alpha= self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                trials.append({'Trial': trial, 'API': api, 'Parameter': 'AUC', 'Mean_Ratio': auc_m, 'Mean_CI_Lower': auc_m_low, 'Mean_CI_Upper': auc_m_up, 'GeoMean_Ratio': auc_g, 'GeoMean_CI_Lower': auc_g_low, 'GeoMean_CI_Upper': auc_g_up})
                trials.append({'Trial': trial, 'API': api, 'Parameter': 'Cmax', 'Mean_Ratio': cmax_m, 'Mean_CI_Lower': cmax_m_low, 'Mean_CI_Upper': cmax_m_up, 'GeoMean_Ratio': cmax_g, 'GeoMean_CI_Lower': cmax_g_low, 'GeoMean_CI_Upper': cmax_g_up})
        trial_df = pd.DataFrame(trials)
    ...
    def regional_bioequiv(self, df  ):
        ...
        for trial in range(self.n_trials):
            sampled_subjects = np.random.choice(subjects, size=self.trial_size, replace=True)
            ...
            for (region, compartment), group_df in df_sample.groupby(['Region', 'Compartment']):
                ...
                for api in apis:
                    ...
                    m, m_low, m_up = self.ratio_and_ci(tvals, rvals, np.mean, self.inner_boot, alpha=self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                    g, g_low, g_up = self.ratio_and_ci(tvals, rvals, self.geometric_mean, self.inner_boot, alpha=self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                    trials.append({'Trial': trial, 'API': api, 'Region': region, 'Compartment': compartment, 'Parameter': 'Regional_AUC', 'Mean_Ratio': m, 'Mean_CI_Lower': m_low, 'Mean_CI_Upper': m_up, 'GeoMean_Ratio': g, 'GeoMean_CI_Lower': g_low, 'GeoMean_CI_Upper': g_up})
                
                e_m, e_mlow, e_mup = self.holistic_effect_ratio_and_ci(all_test, all_ref, np.mean, calculate_effects, self.fevmodel.residual_var, self.inner_boot, alpha=self.alpha_pd, non_parametric = self.non_parametric_pd_flag)
                e_g, e_glow, e_gup = self.holistic_effect_ratio_and_ci(all_test, all_ref, self.geometric_mean, calculate_effects, self.fevmodel.residual_var, self.inner_boot, alpha=self.alpha_pd, non_parametric = self.non_parametric_pd_flag)
                trials.append({'Trial': trial, 'API': 'all', 'Region': region, 'Compartment': compartment, 'Parameter': 'Regional_AUC_Effect', 'Mean_Effect': e_m, 'Mean_Effect_CI_Lower': e_mlow, 'Mean_Effect_CI_Upper': e_mup, 'GeoMean_Effect': e_g, 'GeoMean_Effect_CI_Lower': e_glow, 'GeoMean_Effect_CI_Upper': e_gup})
            ...
            for region_val, group_df in dedup.groupby(['Region']):
                ...
                for api in apis:
                    ...
                    m, m_low, m_up = self.ratio_and_ci(tvals, rvals, np.mean, self.inner_boot, alpha=self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                    g, g_low, g_up = self.ratio_and_ci(tvals, rvals, self.geometric_mean, self.inner_boot, alpha=self.alpha_pk, non_parametric = self.non_parametric_pk_flag)
                    trials.append({'Trial': trial, 'API': api, 'Region': region, 'Compartment': 'ELF', 'Parameter': 'Deposition', 'Mean_Ratio': m, 'Mean_CI_Lower': m_low, 'Mean_CI_Upper': m_up, 'GeoMean_Ratio': g, 'GeoMean_CI_Lower': g_low, 'GeoMean_CI_Upper': g_up})

                e_m, e_mlow, e_mup = self.holistic_effect_ratio_and_ci(all_test, all_ref, np.mean, calculate_effects, self.fevmodel.residual_var, self.inner_boot,  alpha=self.alpha_pd, non_parametric = self.non_parametric_pd_flag )
                e_g, e_glow, e_gup = self.holistic_effect_ratio_and_ci(all_test, all_ref, self.geometric_mean, calculate_effects, self.fevmodel.residual_var, self.inner_boot,  alpha=self.alpha_pd, non_parametric = self.non_parametric_pd_flag)
                trials.append({'Trial': trial, 'API': 'all', 'Region': region, 'Compartment': 'ELF', 'Parameter': 'Deposition_Effect', 'Mean_Effect': e_m, 'Mean_Effect_CI_Lower': e_mlow, 'Mean_Effect_CI_Upper': e_mup, 'GeoMean_Effect': e_g, 'GeoMean_Effect_CI_Lower': e_glow, 'GeoMean_Effect_CI_Upper': e_gup})
