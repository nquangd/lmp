...
def run_deposition_simulations(subjects, products, num_cores = 1, distributed = False):
    ...
    for subject in subjects:
        ...
        geometry_all = subject.GenerationPhysiology
        sub_lung = scale_lung(geometry_all, subject.FRC)

        #subject_intra = subject.get_subject_intra(), if consider intrs for flowprofile
        # Calculate inter-subject scaling factors using a multiplicative approach
        lung_dose_scale_inter = subject.get_factor_inhalation( 'ET', subject.variability.Inter )  #var.lung_dose_inter['mean'] * np.random.lognormal(mean=0, sigma=var.lung_dose_inter['std'])
        mmad_scale_inter = subject.get_factor_inhalation( 'mmad', subject.variability.Inter ) #var.mmad_exMT_inter['mean'] * np.random.lognormal(mean=0, sigma=var.mmad_exMT_inter['std'])
        gsd_scale_inter = subject.get_factor_inhalation( 'gsd', subject.variability.Inter ) # var.gsd_exMT_inter['mean'] * np.random.lognormal(mean=0, sigma=var.gsd_exMT_inter['std'])
        ...
        for product in products:
            for api_data in product.apis_data:
                ...
                lung_dose_factor_intra = subject.get_factor_inhalation( 'ET', subject.variability.Intra )
                mmad_factor_intra = subject.get_factor_inhalation( 'mmad', subject.variability.Intra )
                gsd_factor_intra = subject.get_factor_inhalation( 'gsd', subject.variability.Intra )

                mtdepo = (cfd_mtdepo) * lung_dose_scale_inter * lung_dose_factor_intra * subject.ET_Scale
